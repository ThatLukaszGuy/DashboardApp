...
export default function ExchangeHandler() {
    ...
    <Box backgroundColor={'gray.800'}>
        ...
        <div className='grid grid-cols-1 place-items-center mt-20'>
                <label htmlFor='crypto' className='mt-20 text-white font-thin text-3xl flex'>Search Exchanges <BsSearch size={15}/></label>
                <input 
            ...
            {filteredEx.map((ex,i) => {
                return (
                    <ExchangeCard 
                     ...
                     url={ex.url}
                     image={ex.image}
                     tradingIncentive={ex.has_trading_incentive}
                     trustScore={ex.trust_score}
                     trustScoreRank={ex.trust_score_rank}
                     tradeVolume={ex.trade_volume_24h_btc}
                     tradeVolumeNormalized={ex.trade_volume_24h_btc_normalized}
                     desc={ex.description}
                    />
                )
